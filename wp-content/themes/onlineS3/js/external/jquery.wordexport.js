if (typeof jQuery !== "undefined" && typeof saveAs !== "undefined") {
    (function($) {
        $.fn.wordExport = function(fileName) {
            fileName = typeof fileName !== 'undefined' ? fileName : "online_report";
            var static = {
                mhtml: {
            ...
            for (var i = 0; i < img.length; i++) {
				...
				$('<canvas>').attr("id", "online_report_img_" + i).width(w).height(h).appendTo($(this));
                var canvas = document.getElementById("online_report_img_" + i);
                canvas.width = w;
                canvas.height = h;
